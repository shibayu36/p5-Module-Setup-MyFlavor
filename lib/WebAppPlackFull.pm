...
WebAppPlackFull - pack from webapp-plack-full

=head1 SYNOPSIS

  WebAppPlackFull-setup --init --flavor-class=+WebAppPlackFull new_flavor
...
template: |
  ...
  sub dbconfig {
      my ($self, $name) = @_;
      return config->param("db.$name") or Carp::croak qq(db config for '$name' does not exist);
  }
